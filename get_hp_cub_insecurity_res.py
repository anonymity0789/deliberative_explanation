...
def main():
    ...
    np.save('./cub200/hardness_predictor_res50_lastCovlayer_ComAttr02_recall_multi1.npy', recall)
    np.save('./cub200/hardness_predictor_res50_lastCovlayer_ComAttr02_precision_multi1.npy', precision)
...
def insecurity_extraction(val_loader, grad_cam_hp, grad_cam_cls, imglist, topKcls, com_extracted_attributes, part_Locs, topK_prob_predicted_classes, remaining_mask_size_pool):
    ...
    hardnesslet_info = './cub200/hardnesslet_info.txt'
    fl = open(hardnesslet_info, 'w')
    for i, (input, target, index) in enumerate(val_loader):
