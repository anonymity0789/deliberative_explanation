...
def main():
    ...
    recall, precision = insecurity_extraction(val_hard_loader, secondgrad_hp, grad_cam_cls,
    ...
    np.save('./cub200/hardness_predictor_vgg16_layer42_2ndG_recall_multi1.npy', recall)
    np.save('./cub200/hardness_predictor_vgg16_layer42_2ndG_precision_multi1.npy', precision)
...
def insecurity_extraction(val_loader, secondgrad_hp, grad_cam_cls, imglist, topKcls, com_extracted_attributes, part_Locs, topK_prob_predicted_classes, remaining_mask_size_pool):


    recall = np.ones((len(imglist), np.size(remaining_mask_size_pool)))
    precision = np.ones((len(imglist), np.size(remaining_mask_size_pool)))
    ...
    return np.mean(recall, axis=0), np.mean(precision, axis=0)
