...
parser.add_argument('--lr_step', default='7', help='decreasing strategy')
parser.add_argument('--print-freq', '-p', default=10, type=int,
                    metavar='N', help='print frequency (default: 10)')
parser.add_argument('--resume', default='./cub200/checkpoint_vgg16bn_hp1.pth.tar', type=str, metavar='PATH',
...
def main():
    ...
    np.save('./cub200/all_correct_vgg16_te_multi1.npy', all_correct_te)
    np.save('./cub200/all_predicted_vgg16_te_multi1.npy', all_predicted_te)
    np.save('./cub200/all_class_dis_vgg16_te_multi1.npy', all_class_dis_te)

    all_correct_te = np.load('./cub200/all_correct_vgg16_te_multi1.npy')
    all_predicted_te = np.load('./cub200/all_predicted_vgg16_te_multi1.npy')
    all_class_dis_te = np.load('./cub200/all_class_dis_vgg16_te_multi1.npy')


    hardness_scores_te, hardness_te_idx_each = save_predicted_hardness(train_loader, val_loader, model_ahp_trunk, model_ahp_hp)
    np.save('./cub200/hardness_scores_te_vgg16_multi1.npy', hardness_scores_te)
    np.save('./cub200/hardness_te_idx_each_vgg16_multi1.npy', hardness_te_idx_each)

    hardness_scores_te = np.load('./cub200/hardness_scores_te_vgg16_multi1.npy')
    hardness_te_idx_each = np.load('./cub200/hardness_te_idx_each_vgg16_multi1.npy')
    ...
    # data loader
    assert callable(datasets.__dict__['cub200hard_moved'])
    get_dataset = getattr(datasets, 'cub200hard_moved')
    num_classes = datasets._NUM_CLASSES['cub200hard_moved']
    _, val_hard_loader_moved = get_dataset(
        batch_size=1, num_workers=args.workers)



    remaining_mask_size_pool = np.arange(0.01, 1.0, 0.01)
    IOU_points = insecurity_extraction(val_hard_loader_moved, grad_cam_hp, grad_cam_cls,
    ...
    np.save('./cub200/hardness_predictor_vgg16bn_layer42_IG_robustComp_multi1.npy', IOU_points)
...
class GradCam_hp:
    ...
    def __call__(self, input, refer_input, step):
        ...
        cam_positive = (target - refer_features[-1].cpu().data.numpy()[0, :]) * mask_positive
...
class GradCam_cls:
    ...
    def __call__(self, input, TopKclass = 5, topK_prob_predicted_classes=None, refer_input=None, step=50):
        ...
        for i_cls in range(np.size(topK_prob_predicted_classes)):
            cam_positive = (target - refer_features[-1].cpu().data.numpy()[0, :]) * all_grads_val[:, :, :, i_cls]
            cam_positive = np.sum(cam_positive, axis=0)
...
def insecurity_extraction(val_loader, grad_cam_hp, grad_cam_cls, imglist, topKcls, com_extracted_attributes, part_Locs, topK_prob_predicted_classes, remaining_mask_size_pool):
    ...
    IOU_total = np.zeros((len(imglist), np.size(remaining_mask_size_pool)))

    for i, (input, input_moved, target, index) in enumerate(val_loader):
        ...
        hardness_heatmaps = grad_cam_hp(input, refer_img, 50)
        hardness_mask = np.copy(hardness_heatmaps)
        hardness_mask[hardness_mask > 0] = 1
        classifier_heatmaps = grad_cam_cls(input, 200, topK_prob_predicted_classes[i, :], refer_img, 50)
        classifier_heatmaps[classifier_heatmaps < 0] = 1e-7

        # randomly move input_moved
        x_moved = randrange(-5, 5)
        y_moved = randrange(-5, 5)

        input_moved = input_moved[:, :, (16 + x_moved):(256 - 16 + x_moved),
                      (16 + y_moved):(256 - 16 + y_moved)]  # this is equal to translate input randomly 1 to 5 pixels

        hardness_heatmaps_moved = grad_cam_hp(input_moved, refer_img, 50)
        hardness_mask_moved = np.copy(hardness_heatmaps_moved)
        hardness_mask_moved[hardness_mask_moved > 0] = 1
        classifier_heatmaps_moved = grad_cam_cls(input_moved, 200, topK_prob_predicted_classes[i, :], refer_img, 50)
        classifier_heatmaps_moved[classifier_heatmaps_moved < 0] = 1e-7
        ...
        part_Locs_example_moved = np.zeros(part_Locs_example.shape)
        part_Locs_example_moved[np.where(part_Locs_example.any(axis=1))[0], 0] = part_Locs_example[np.where(part_Locs_example.any(axis=1))[0], 0] - y_moved
        part_Locs_example_moved[np.where(part_Locs_example.any(axis=1))[0], 1] = part_Locs_example[np.where(part_Locs_example.any(axis=1))[0], 1] - x_moved
        part_Locs_example_moved = part_Locs_example_moved.astype(int)
        ...
        for i_cls in range(topKcls):
            for j_cls in range(i_cls + 1, topKcls):
                cur_misclass_pairs = np.concatenate((np.reshape(confusion_classes[:, :, i_cls].squeeze(), (-1, 1)),
                                                     np.reshape(confusion_classes[:, :, j_cls].squeeze(), (-1, 1))),
                                                    axis=1)
        ...
        confusion_classes_moved = np.argsort(classifier_heatmaps_moved, axis=2)[:, :, -topKcls:]
        confusion_classes_moved = np.sort(confusion_classes_moved, axis=2)
        confusion_classes_moved = confusion_classes_moved * np.repeat(hardness_mask_moved[:, :, np.newaxis], topKcls, axis=2)

        misclass_pairs_moved = np.zeros((1, 2))
        for i_cls in range(topKcls):
            for j_cls in range(i_cls + 1, topKcls):
                cur_misclass_pairs_moved = np.concatenate((np.reshape(confusion_classes_moved[:, :, i_cls].squeeze(), (-1, 1)),
                                                     np.reshape(confusion_classes_moved[:, :, j_cls].squeeze(), (-1, 1))),
                                                    axis=1)
                misclass_pairs_moved = np.concatenate((misclass_pairs_moved, cur_misclass_pairs_moved), axis=0)

        misclass_pairs_moved = np.unique(misclass_pairs_moved, axis=0)
        misclass_pairs_moved = misclass_pairs_moved[~np.all(misclass_pairs_moved == 0, axis=1)]
        misclass_pairs_moved = misclass_pairs_moved.astype(int)

        atom_num_moved = np.size(misclass_pairs_moved, axis=0)
        atom_mask_moved = np.zeros(np.size(hardness_mask_moved))


        # find the union misclass_pairs
        union_misclass_pairs = np.concatenate((misclass_pairs, misclass_pairs_moved), axis=0)
        union_misclass_pairs = np.unique(union_misclass_pairs, axis=0)

        atom_num = np.size(union_misclass_pairs, axis=0)
        atom_mask = np.zeros(np.size(hardness_mask))
        ...
        for i_remain in range(np.size(remaining_mask_size_pool)):
            ...
            for i_atom in range(atom_num):
                ...
                if (union_misclass_pairs[i_atom, :] in misclass_pairs) and (union_misclass_pairs[i_atom, :] in misclass_pairs_moved):

                    cur_atom_mask1 = np.zeros((np.size(classifier_heatmaps, 0), np.size(classifier_heatmaps, 0), topKcls))
                    cur_atom_mask1[np.where(confusion_classes[:, :, :] == union_misclass_pairs[i_atom, 0])] = 1
                    ...
                    cur_atom_mask2[np.where(confusion_classes[:, :, :] == union_misclass_pairs[i_atom, 1])] = 1
                    cur_atom_mask2 = np.sum(cur_atom_mask2, axis=-1)
                    atom_mask = cur_atom_mask1 * cur_atom_mask2

                    hardness_atom = classifier_heatmaps[:, :, union_misclass_pairs[i_atom, 0]].squeeze() * classifier_heatmaps[:, :, union_misclass_pairs[i_atom, 1]].squeeze() * hardness_heatmaps
                    ...
                    com_attributes = com_extracted_attributes[topK_prob_predicted_classes[i, union_misclass_pairs[i_atom, 0]], topK_prob_predicted_classes[i, union_misclass_pairs[i_atom, 1]]]
                    ...
                    cur_atom_mask1_moved = np.zeros((np.size(classifier_heatmaps_moved, 0), np.size(classifier_heatmaps_moved, 0), topKcls))
                    cur_atom_mask1_moved[np.where(confusion_classes_moved[:, :, :] == union_misclass_pairs[i_atom, 0])] = 1
                    cur_atom_mask1_moved = np.sum(cur_atom_mask1_moved, axis=-1)
                    cur_atom_mask2_moved = np.zeros((np.size(classifier_heatmaps_moved, 0), np.size(classifier_heatmaps_moved, 0), topKcls))
                    cur_atom_mask2_moved[np.where(confusion_classes_moved[:, :, :] == union_misclass_pairs[i_atom, 1])] = 1
                    cur_atom_mask2_moved = np.sum(cur_atom_mask2_moved, axis=-1)
                    atom_mask_moved = cur_atom_mask1_moved * cur_atom_mask2_moved

                    hardness_atom_moved = classifier_heatmaps_moved[:, :, union_misclass_pairs[i_atom, 0]].squeeze() * classifier_heatmaps_moved[:, :,
                                                                                                                       union_misclass_pairs[
                                                                                                         i_atom, 1]].squeeze() * hardness_heatmaps_moved
                    hardness_atom_moved = cv2.resize(hardness_atom_moved, (224, 224))
                    hardness_atom_mask_moved = np.copy(hardness_atom_moved)

                    threshold_moved = np.sort(hardness_atom_mask_moved.flatten())[int(-remaining_mask_size * 224 * 224)]
                    hardness_atom_mask_moved[hardness_atom_mask_moved > threshold_moved] = 1
                    hardness_atom_mask_moved[hardness_atom_mask_moved < 1] = 0

                    all_attributes_positions_moved = np.zeros((224, 224))
                    common_attributes_positions_moved = np.zeros((224, 224))

                    com_attributes_moved = com_extracted_attributes[
                        topK_prob_predicted_classes[i, union_misclass_pairs[i_atom, 0]], topK_prob_predicted_classes[
                            i, union_misclass_pairs[i_atom, 1]]]


                    com_attributes_moved = np.array(com_attributes_moved)

                    part_Locs_example_copy_moved = np.copy(part_Locs_example_moved)
                    part_Locs_example_copy_moved = part_Locs_example_copy_moved[~np.all(part_Locs_example_copy_moved == 0, axis=1)]
                    all_attributes_positions_moved[part_Locs_example_copy_moved[:, 1], part_Locs_example_copy_moved[:, 0]] = 1

                    common_attributes_positions_moved[part_Locs_example_moved[com_attributes_moved, 1], part_Locs_example_moved[com_attributes_moved, 0]] = 1
                    common_attributes_positions_moved[0, 0] = 0
                    if np.sum(common_attributes_positions_moved) < 1:  # means no ground truth common attribute position
                        noeffect_atom = noeffect_atom + 1
                        continue

                    original_mask = np.zeros((256, 256))
                    original_mask[16:(256-16), 16:(256-16)] = hardness_atom_mask*common_attributes_positions

                    original_mask_moved = np.zeros((256, 256))
                    original_mask_moved[(16+x_moved):(256-16+x_moved), (16+y_moved):(256-16+y_moved)] = hardness_atom_mask_moved*common_attributes_positions_moved

                    original_mask_sum = np.zeros((256, 256))
                    original_mask_sum[(original_mask + original_mask_moved)>0] = 1


                    if np.sum(original_mask_sum) > 0:
                        IOU_com_points = np.sum(original_mask * original_mask_moved) / np.sum(original_mask_sum)
                    else:
                        IOU_com_points = 1

                    total_IOU_i = total_IOU_i + IOU_com_points

                else:
                    noeffect_atom = noeffect_atom + 1


            IOU_total[i, i_remain] = total_IOU_i / (atom_num - noeffect_atom)
    return np.nanmean(IOU_total, axis=0)
