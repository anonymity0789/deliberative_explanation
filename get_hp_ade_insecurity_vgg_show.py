...
def main():
    ...
    print(IOU)
    np.save('./ade/hardness_predictor_vgg16bn_layer42_IOU_multi1.npy', IOU)
...
def insecurity_extraction(val_loader, grad_cam_hp, grad_cam_cls, imglist, topKcls, com_extracted_attributes, seg_list, topK_prob_predicted_classes, remaining_mask_size_pool):
    ...
    insecurities_info = './ade/insecurities_info.txt'
    ...
    for i, (input, target, index) in enumerate(val_loader):
        ...
        for i_remain in range(np.size(remaining_mask_size_pool)):
            ...
            for i_atom in range(atom_num):
                ...
                seg = show_segment_on_image(img, hardness_atom_mask)
                info = imglist[i].split("/")
                name = info[-1]
                if not os.path.exists("./ade/insecurities/"  + str(i)):
                     os.makedirs("./ade/insecurities/" +  str(i))
                cv2.imwrite("./ade/insecurities/" + str(i) + "/" + str(remaining_mask_size) + "_" + str(topK_prob_predicted_classes[i, misclass_pairs[i_atom, 0]]) + "_" + str(topK_prob_predicted_classes[i, misclass_pairs[i_atom, 1]]) + "_" + name, seg)
