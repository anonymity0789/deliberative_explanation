...
def main():
    ...
    integrad_hp = InteGrad_hp(model_ahp_trunk, model_ahp_hp, target_layer_names=["42"], use_cuda=True)
    integrad_cls = InteGrad_cls(model_main, target_layer_names=["42"], use_cuda=True)
    ...
    recall, precision = insecurity_extraction(val_hard_loader, integrad_hp, integrad_cls,
    ...
    np.save('./cub200/hardness_predictor_vgg16_layer42_IG_recall_multi1.npy', recall)
    np.save('./cub200/hardness_predictor_vgg16_layer42_IG_precision_multi1.npy', precision)
...
def insecurity_extraction(val_loader, integrad_hp, integrad_cls, imglist, topKcls, com_extracted_attributes, part_Locs, topK_prob_predicted_classes, remaining_mask_size_pool):
    ...
    for i, (input, target, index) in enumerate(val_loader):
        ...
        hardness_heatmaps = integrad_hp(input, refer_img, 50)
        hardness_mask = np.copy(hardness_heatmaps)
        hardness_mask[hardness_mask > 0] = 1
        classifier_heatmaps = integrad_cls(input, 200, topK_prob_predicted_classes[i, :], refer_img, 50)
    ...
    return np.mean(recall, axis=0), np.nanmean(precision, axis=0)
