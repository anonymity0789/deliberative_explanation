...
parser.add_argument('--resume', default='./ade/checkpoint_pretrain_res50.pth.tar', type=str, metavar='PATH',
...
def main():
    ...
    remain_bool_te = np.zeros([REPEAT_NUM, 1984])
    hardness_tr = np.zeros([REPEAT_NUM, 18188])
    hardness_tr_idx = np.zeros([REPEAT_NUM, 18188])
    hardness_te = np.zeros([REPEAT_NUM, 1984])
    hardness_te_idx = np.zeros([REPEAT_NUM, 1984])
    ...
    for j in range(REPEAT_NUM):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            cur_AUC = np.trapz(all_accuracy_each)

            if cur_AUC > AUC:
                AUC = cur_AUC
            print(cur_AUC)
            print(AUC)
            if AUC > threshold or (total_cor_num_cor / np.sum(all_correct_te) > 0.4 and total_cor_num_incor / (5794 - np.sum(all_correct_te)) > 0.4):
                ...
                }, filename='./ade/checkpoint_res50_hp.pth.tar')
