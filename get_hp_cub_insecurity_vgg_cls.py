...
def main():
    ...
    np.save('./cub200/hardness_predictor_vgg16bn_layer42_recall_cls_multi1.npy', recall)
    np.save('./cub200/hardness_predictor_vgg16bn_layer42_precision_cls_multi1.npy', precision)
    ...
    for i, (input, target, index) in enumerate(val_loader):
        ...
        for i_remain in range(np.size(remaining_mask_size_pool)):
            ...
            for i_atom in range(atom_num):
                ...
                hardness_atom = classifier_heatmaps[:, :, misclass_pairs[i_atom, 0]].squeeze() * classifier_heatmaps[:, :, misclass_pairs[i_atom, 1]].squeeze() * atom_mask
