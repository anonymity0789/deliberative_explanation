...
def main():
    ...
    recall, precision = insecurity_extraction(val_hard_loader, grad_cam_hp, grad_cam_cls,
    ...
    np.save('./cub200/hardness_predictor_vgg16bn_layer42_recall_multi1.npy', recall)
    np.save('./cub200/hardness_predictor_vgg16bn_layer42_precision_multi1.npy', precision)
...
def show_segment_on_image(img, mask, com_attributes_positions=None, all_attributes_positions=None, is_cls=True):
    ...
    if is_cls == False:
        ...
        if np.sum(all_attributes_positions * mask[:, :, 0]) > 0:
...
def insecurity_extraction(val_loader, grad_cam_hp, grad_cam_cls, imglist, topKcls, com_extracted_attributes, part_Locs, topK_prob_predicted_classes, remaining_mask_size_pool):
    ...
    insecurities_info = './cub200/insecurities_info_includeError.txt'
    fl = open(insecurities_info, 'w')
    for i, (input, target, index) in enumerate(val_loader):
        ...
        for i_remain in range(np.size(remaining_mask_size_pool)):
            ...
            for i_atom in range(atom_num):
                ...
                seg = show_segment_on_image(img, hardness_atom_mask, common_attributes_positions, all_attributes_positions, is_cls=False)
                ...
                example_info = imglist[i] + " " + str(remaining_mask_size) + " " + str(topK_prob_predicted_classes[i, misclass_pairs[i_atom, 0]]) + " " + str(topK_prob_predicted_classes[i, misclass_pairs[i_atom, 1]]) + " " + str(com_attributes)
                fl.write(example_info)
                fl.write("\n")
    ...
    fl.close()
    print(np.nanmean(precision, axis=1))
