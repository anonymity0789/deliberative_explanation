...
def main():
    ...
    np.save('./cub200/hardness_predictor_vgg16bn_layer42_2ndG_robustComp_multi1.npy', IOU_points)
...
class GradCam_hp:
    ...
    def __call__(self, input):
        ...
        # secondresponse[secondresponse < 0.0] = 0.0
...
def insecurity_extraction(val_loader, grad_cam_hp, grad_cam_cls, imglist, topKcls, com_extracted_attributes, part_Locs, topK_prob_predicted_classes, remaining_mask_size_pool):
    ...
    for i, (input, input_moved, target, index) in enumerate(val_loader):
        ...
        input_moved = input_moved[:, :, (16+x_moved):(256-16+x_moved), (16+y_moved):(256-16+y_moved)]  # this is equal to translate input randomly 1 to 5 pixels

        hardness_heatmaps_moved = grad_cam_hp(input_moved)
        hardness_mask_moved = np.copy(hardness_heatmaps_moved)
        hardness_mask_moved[hardness_mask_moved > 0] = 1
        classifier_heatmaps_moved = grad_cam_cls(input_moved, 200, topK_prob_predicted_classes[i, :])
